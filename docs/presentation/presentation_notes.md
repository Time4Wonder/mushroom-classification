...
1. **odor (Geruch):** Cramers's V = 0,971 -- fast perfekte Trennung. Aber: Geruch
...
3. **gill_color (Lamellenfarbe):** Wurde **nicht** entfernt, weil:
   ...
   - Cramers's V = 0,68 -- stark, aber ohne perfekte Trennung auf allen Levels
...
**Berechnung:** Cramers's V ist ein Maß für den Zusammenhang zweier nominaler
...
**Plot:** Der Cramers's V Plot (balkendiagramm) zeigt, wie rapide die Werte
...
L(beta) = Produkt  p(x_i)^y_i . (1-p(x_i))^(1-y_i)

mit p(x) = 1 / (1 + e^(-x^Tbeta))
...
wird. Gini = 1 - Summe  p_k^2, wobei p_k der Anteil der Klasse k im Knoten ist.
...
ist 10x schlimmer als ein harmloser." Ein höheres Verhältnis (50:1, 100:1)
...
- gill_color (691), gill_size (389), stalk_surface_above_ring (303),
  ring_type (286), population (247) -- bestätigt Cramers's V
...
- "Warum haben Sie gill_color nicht auch entfernt?" -> Es ist ein Standard-
  ...
  (Cramers's V = 0,68) ist hoch, aber nicht deterministisch.
